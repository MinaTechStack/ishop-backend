        ...
        const fetchDashboardData = async () => {
            try {
                ...
                if (err.response && (err.response.status === 401 || err.response.status === 403)) {
                    console.log("Client-side Dashboard: Data fetch failed due to authentication. Redirecting.");
                    localStorage.removeItem('admin_token_fallback'); // Clean up old fallback if any
        ...
        <div className="p-6 bg-gray-100 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-4 rounded shadow">
                    ...
                    {paymentData.some(data => data.value > 0) ? (
                        <ResponsiveContainer width="100%" height={250}>
                            <PieChart>
                                <Pie
                                    data={paymentData}
                        ...
                        <p className="text-center text-gray-500">No payment method data available.</p>
                    )}
                </div>
            </div>
